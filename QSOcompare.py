...
def read_icrf2():
    ...
    = np.loadtxt('../data/icrf2.dat', usecols=range(4,13,1), unpack=True)
...
def read_qso():
    ...
    = np.loadtxt('../data/qso.dat', usecols=range(3,8), unpack=True, delimiter='|')
...
for j in range(N):
    ...
    if len(i):
        ...
        print>>flog, 'No corresponding entry for %s are found in icrf2.'%ICRFg[j]
...
## offset, gaia - icrf2
d_RA = (RAg - RAi)*np.cos(np.deg2rad(DEg/3600))*1.0e3         ##  mas
